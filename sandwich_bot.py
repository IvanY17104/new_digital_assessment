...
names = [
    ...
    "Grace", "Amy", "Alice", "Lara",  "Rayyan"]  # List of random names
...
def not_blank(question):
    # a not blank function to check if something isnt blank
    valid = False
    while not valid:  # a loop
        response = input(question)
        # asks the question that is stated, and allows input
        if response != "":
            return response.title()  # if input isn't blank returns it back,
            # and it makes the input's first letter have capital
        else:
            print("Sorry this cannot be blank")
            # if input is blank this is printed
    ...
    while True:  # this creates a loop, so things are repeated if there are
            try:
                ...
                if num >= low and num <= high:
                    return num  # if the number is an appropriote number
                    # between what we need it returns it
                else:
                    print(f"The number must be between {low} or {high}")
                    # prints this when a number that we don't want is inputted
...
def welcome():  # Created a function for welcome
    '''
    Purpose: To generate a random name
    from the list and print out a welcome message
    Parameters: none
    Returns: none
    '''

    num = randint(0, 9)  # num will give a random
    # number (randint) between 0 to 9
    name = (names[num])  # each name in names has a number associated with it,
    # and num  will give a random number, so 'name' will give a random name
    print("Welcome to Ivan's Sandwich Store!")
    print("My name is", name, "and I will help you take your Sandwich orders.")
    # This is the welcome message that will show up


# Menu for pickup or delivery
def order_type():
    del_pick = ""
    question = (f"Please enter a number ")
    # confirms low/high for this function
    ...
    if delivery == 1:
        ...
        delivery_info()
        # will activate delivery function and begin asking things for delivery
        del_pick = "delivery"
        # if delivery is picked del_pick variable becomes delivery

    else:
        print("Pickup")  # if 2 is inputted it prints pickup
        pickup_info()
        # will activate pickup function and begin asking things for pickup
        del_pick = "pickup"
        # if pickup is picked del_pick variable becomes pickup
...
def delivery_info():
    ...
    # asks the user for their house number and allows input
...
def menu():
    ...
    for count in range(number_sandwich):
        print("{} {} ${:.2f}" .format(
            count+1, sandwich_names[count],
            sandwich_prices[count]))
        # the 3 {} brackets contain count sandwich name and price count,
        # so things that we do in those affect the count print
        # dollar sign and :.2f makes the sandwich prices have a dollar
        # sign infront, and have 2 decimal places
        # count +1 makes it so that the index numbers start at 1 and not 0


# Choose total number of items - max 5 & Sandwich Ordering - from menu -
...
def order_sandwich():
    ...
    for item in range(num_sandwiches):
        while num_sandwiches > 0:
            print(
                "Please choose your sandwiches by entering the "
                "number from the menu  ")
            question = ("Please enter a number "
                        f"between {menu_low} and {menu_high} ")
            sandwich_ordered = val_int(menu_low, menu_high, question)
            # asks user to enter which pizza they want, validats integer
            sandwich_ordered = sandwich_ordered - 1
            # this is so that it doesn't print the wrong things since
            # we ahve the index numbers starting at 1 instead of 0
            order_list.append(sandwich_names[sandwich_ordered])
            order_cost.append(sandwich_prices[sandwich_ordered])
            # the inpuuted number is then sent to the sandwich names
            # and prices which sends back the respective index number items.
            print(
                "{} ${:.2f}".format(sandwich_names[sandwich_ordered],
                                    sandwich_prices[sandwich_ordered]))
            # prints the sandwich names and prices together,
            # prices have a dollar sign and are 2 d.p
            num_sandwiches = num_sandwiches-1
            # this is what helps create the loop by taking 1 away
            # from num_sandwiches for every one that is ordered


# Print order out - including if order is delivery or pickup
# and names and price of each item - total cost
def print_order(del_pick):
    total_cost = sum(order_cost)
    # total_cost is the sum of all prices in order_cost
    print()
    ...
    if del_pick == "delivery":
        ...
        print(
            f"Name: {customer_details['name']} \nPhone: "
            f"{customer_details['phone']}")
        # if del_pick is delivery it prints all 5 things,
        # if del_pick is pickup it only prints the two
    print()
    print("Order Details")
    count = 0
    ...
    print()
    # this prints out the total cost of the order properly,
    # with a $ sign and 2 d.p


# Ability to cancel or proceed with order
def confirm_cancel():
    question = (f"Please enter a number ")
    # confirms the high/low for this function
    print ("Please confirm your order")
    print ("To confirm please enter 1")
    print ("To cancel please enter 2")
    # Asks the users what they want
    ...
    if confirm == 1:
            print(
                ...
                "sent to the kitchen.")
            # if 1 is inputted it prints order confirmed
            new_exit()
    elif confirm == 2:
            print(
                "Your Order has been Cancelled, "
                "you may restart your order or exit the bot")
            # if 2 is inputted it prints order cancelled
            new_exit()
...
def new_exit():
    ...
    if confirm == 1:
        ...
        # if 1 is inputted it prints another order will be created
        order_list.clear()
        order_cost.clear()
        customer_details.clear()
        # clears all inputs that were put into these lists/dictionaries
        main()  # runs main again, restarting bot

    elif confirm == 2:
        print("You will be exited from the bot")
        # if 2 is inputted it prints you will be exited from bot
        order_list.clear()
        order_cost.clear()
        customer_details.clear()
        # clears all inputs that were put into these lists/dictionaries
...
def main():  # Created a function to run all the functions
    ...
    welcome()
    del_pick = order_type()
    # del_pick is a variable that will come from order_type
    menu()
    order_sandwich()
    print_order(del_pick)
    # when print_order function happens the del_pick variable is sent there
    confirm_cancel()
